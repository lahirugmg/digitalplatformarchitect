...
import { CHECKLIST_ITEMS, type ChecklistResult } from '@/lib/operational-sympathy'
...
export function ReportExport({ result, scores }: ReportExportProps) {
  ...
  const generateMarkdownReport = (): string => {
    ...
    report += `| Element | Weight | Score (0-5) | Weighted |\n`
    report += `|---------|--------|-------------|----------|\n`

    for (const item of CHECKLIST_ITEMS) {
      const score = scores[item.id] ?? 0
      const weightedScore = ((score / 5) * item.weight).toFixed(1)
      report += `| ${item.element} | ${item.weight}% | ${score} | ${weightedScore} |\n`
    }
    ...
    if (result.topRisks.length === 0) {
      ...
      for (const risk of result.topRisks.slice(0, 5)) {
        const item = CHECKLIST_ITEMS.find((candidate) => candidate.id === risk.itemId)
        if (!item) {
          continue
        }
        report += `- **${item.element}**: potential gain +${risk.weightedGap.toFixed(1)} (gap ${risk.gap}/5)\n`
      }
      report += '\n'
    }

    report += `---\n\n`
    report += `*Generated by Digital Platform Architect - Operational Sympathy Scorecard*\n`
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 sm:p-6">
      ...
      <p className="mt-2 text-sm text-slate-600">Download or copy this scorecard for architecture reviews and handoff docs.</p>
