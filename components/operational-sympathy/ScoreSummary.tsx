...
import { CHECKLIST_ITEMS, getScoreColor, type ChecklistResult } from '@/lib/operational-sympathy'
...
export function ScoreSummary({ result, onReset }: ScoreSummaryProps) {
  ...
  const byId = new Map(CHECKLIST_ITEMS.map((item) => [item.id, item]))
    ...
    <section className={`rounded-xl border ${colors.border} ${colors.bg} p-5 sm:p-6`} aria-live="polite">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Weighted across all nine elements</p>
        </div>
        <button
          onClick={onReset}
        ...
        {Object.entries(result.categoryScores).map(([category, { score, max }]) => {
          ...
          return (
            <div key={category} className="rounded-md border border-slate-200 bg-white p-2.5">
              <div className="text-[11px] font-medium capitalize text-slate-600">{category}</div>
        ...
        {topThreeRisks.length === 0 ? (
          ...
          <ul className="mt-2 space-y-2 text-sm text-slate-700">
            {topThreeRisks.map((risk) => {
              const item = byId.get(risk.itemId)
              return (
                <li key={risk.itemId} className="rounded-md border border-slate-200 bg-white p-2.5">
                  <p className="font-medium text-slate-900">{item?.element ?? risk.itemId}</p>
